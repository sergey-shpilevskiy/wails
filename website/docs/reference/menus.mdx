...
```go title="Package: github.com/sergey-shpilevskiy/wails/v2/pkg/menu"
type Menu struct {
	Items []*MenuItem
}
```
...
an Accelerator as a combination or key + [Modifier](#modifier). They are available in the `"github.com/sergey-shpilevskiy/wails/v2/pkg/menu/keys"` package.

Example:

```go title="Package: github.com/sergey-shpilevskiy/wails/v2/pkg/menu/keys"
